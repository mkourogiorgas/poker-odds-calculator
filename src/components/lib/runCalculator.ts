...
const runPokerOdds = async (deck: Hand, players: Players): Promise<Results> => {
  ...
  const chunks = U.splitArrayToChunks(communities, Math.ceil(communities.length / 4));

  const splitResults = await Promise.all(chunks.map((chunk) => U.runWorker(chunk, activePlayers)));
