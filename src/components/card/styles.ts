...
const medium = 'w-9.75 h-13 text-xs';
...
const cardBackground = (suit: string) => suitBackgroundColor[suit] || suitBackgroundColor.spades;
const cardHover = (isInTable?: boolean) => (isInTable ? state.hover : '');
const cardOpacity = (isSelected: boolean) => (isSelected ? state.selected : state.available);
const cardSymbol = (suit: string) => suitSymbol[suit] || suitSymbol.spades;
const cardText = (suit: string) => suitTextColor[suit] || suitTextColor.spades;
